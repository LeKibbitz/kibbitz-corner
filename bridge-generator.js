 ...
 * - Algorithmes Mitchell 1-4-7 et équilibré
 ...
 * @param {string} algo - Algorithme ('1-4-7' ou 'balanced')
 */
function setAlgorithm(algo) {
    ...
    if (parsedPairs && parsedPairs.length > 0) {
        ...
        renderMitchellDisplay();
        showStatus(`Algorithme ${algo === '1-4-7' ? 'Mitchell 1-4-7' : 'Équilibré'} appliqué`, 'success');
    } else {
...
function generateTestData80() {
    // Implementation similaire avec 80 paires...
    return 'Test data for 80 pairs...'; // Simplified for brevity
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (generateBtn) {
        generateBtn.addEventListener('click', function(e) {
            e.preventDefault();
            console.log('🔍 DEBUG: Generate button clicked via event listener');
            generateSections();
        });
    ...
    buttons.forEach(button => {
        ...
        if (onclickAttr) {
            button.addEventListener('click', function(e) {
                console.log('🔍 DEBUG: Button clicked (backup listener):', onclickAttr);
                try {
                    eval(onclickAttr);
                } catch (error) {
    ...
    console.log('✓ Backup event listeners added for', buttons.length, 'buttons');
